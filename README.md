...
- 💬 **企业微信**: 全面覆盖三种接入模式 — **企业微信机器人**（自建应用消息推送）、**企业微信智能机器人**（内置 AI 助手）及 **企业微信客服**（外部客户服务），实现私域流量全场景覆盖。
- 🦅 **飞书机器人**: 对接飞书开放平台，支持长连接模式、事件订阅与富文本消息交互。
- 📚 **文档更新**: 同步上线全系列机器人集成指引，涵盖各平台详细配置流程（详见文档中心“技术细节”章节）。

### 2026-02-09 🤖 LangGraph Agentic RAG 与多轮自主检索
- 🔧 **LangGraph 集成**: 采用 LangGraph 1.x 重构聊天功能，支持工具调用模式。
- 🛠️ **RAG 工具化**: 将知识库检索封装为 `search_knowledge_base` 工具，AI 自主判断是否需要调用知识库。
