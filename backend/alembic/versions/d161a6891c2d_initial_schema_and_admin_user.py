...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("domain", sa.String(length=200), nullable=True, comment="站点域名"),
...
def _create_default_admin_user() -> None:
    ...
    密码: admin123
    """
    # 创建 users 表的引用（DateTime 需要指定 timezone=True 以匹配实际表定义）
    ...
    conn = op.get_bind()
    result = conn.execute(
        sa.text("SELECT COUNT(*) FROM users WHERE email = :email"), {"email": "admin@example.com"}
    )
    count = result.scalar()
...
def _delete_default_admin_user() -> None:
    ...
    print("🗑️  删除默认管理员用户：admin@example.com")
...
def _create_demo_site() -> None:
    ...
    conn = op.get_bind()
    result = conn.execute(
        sa.text("SELECT COUNT(*) FROM sites WHERE domain = :domain"), {"domain": "medical"}
    )
    count = result.scalar()
    ...
    if count == 0:
        ...
        print("✅ 创建 Demo 站点及其快速问题配置")
    else:
        print("⚠️  Demo 站点已存在，跳过创建")
