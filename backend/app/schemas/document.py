...
class CollectionInfo(BaseModel):
    ...
    ancestors: list[CollectionAncestor] = Field(
        default_factory=list, description="祖先合集链（从根到父级）"
    )
...
class DocumentBase(BaseModel):
    ...
    status: DocumentStatus = Field(
        default=DocumentStatus.DRAFT, description="状态: published, draft"
    )
    vector_status: VectorStatus = Field(
        default=VectorStatus.NONE,
        description="向量化状态: none, pending, processing, completed, failed",
    )
